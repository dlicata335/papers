...
\[
\inferfour
  {\deqdepglob{\Gamma}{(\dhomj{H}{N_1}{N_2}{\blank})}{\tel{M}}
   {(\dhomj{K}{N_1'}{N_2'}{\blank})}{\tel{M'}}}
  {\deqdepglob{\Gamma}{H}{\lhs{\tel M}}{K}{\lhs{\tel M'}}}
  {\deqdepglob{\Gamma}{H}{\rhs{\tel M}}{K}{\rhs{\tel M'}}}
  {\deqofdepglob{\Gamma}{N_1}{N_1'}{H}{\lhs{\tel M}}}
  {\deqofdepglob{\Gamma}{N_2}{N_2'}{H}{\rhs{\tel M}}}
\]
...
\FIXME{Is the telescopic ap defined in terms of the dependent ap?}
...
\[\infer
  {\wfdepglob{\Gamma}{(\Apd{(\tptm xB.N_1)}{H})}{\tel A,B.C}{(\tel G,H)}}
  {\ofglob{\Gamma,\tptm xB}{N_1}{C}
  &\wfglob{\Gamma}{\tel G}{\tel A}{n}
  &\wfdepglob{\Gamma}{H}{\tel A.B}{\tel G}}
\]
...
\[\begin{aligned}
  \Apd{(\tptm xB.N_1)}{(\tel A.B)} & := (\tel A,B).C\\
  \Apd{(\tptm xB.N_1)}{(\dhomj{H}{N}{N'}{\blank})} & :=
    \dhomj
      {\Apd{(\tptm xB.N_1)}{H}}
      {\apd{(\tptm xB.N_1)}{N}}
...
We have the following definitional equality when $N_2$ is a 0-cell:
\[ \apd{(\tptm xB.N_1)}{N_2} \equiv N_1[N_2/x] \]
In this case, the types are equal because for $N_2:(\teltm xA.\tptm
xB)\downarrow \tel M$,
\[ \Apd{(\tptm xB.N_1)}{(\teltm xA.\tptm xB)} := (\teltm xA,\tptm xB).C \]
and it suffices to observe that $C[\tel M/\tel x,N_2/x]\equiv (C[N_2/x])[\tel
M/\tel x]$.
...
\dhomj{\Fstsigma H}{\apd{(x.\fst x)}{N_1}}{\apd{(x.\fst x)}{N_2}}{\blank}
&\\
\Sndsigma{\tel A.\sigmacl xAB} &:= \tel A,A.B \\
\Sndsigma{\dhomj{H}{N_1}{N_2}{\blank}} &:=
\dhomj{\Sndsigma H}{\apd{(x.\snd x)}{N_1}}{\apd{(x.\snd x)}{N_2}}{\blank}
\end{aligned}\]

To see that these operations are well-typed, we must show two things:

\begin{lem}
$\deqdepglob\Gamma{\Fstsigma H}{\tel M}{\Apd{(x.\fst x)}H}{(\tel M,N)}$ for any
$\ofdepglob\Gamma NH{\tel M}$.
\end{lem}
\begin{proof}
There are two cases. If $H=\teltm xA.(\sigmacl xAB)$,
\[\begin{aligned}
\Fstsigma{\teltm xA.(\sigmacl xAB)}\downarrow\tel M
&:= \teltm xA.A\downarrow\tel M \\
\Apd{(x.\fst x)}{(\teltm xA.(\sigmacl xAB))}\downarrow(\tel M,N)
...
\[\begin{aligned}
\Fstsigma{\dhomj H{N_1}{N_2}\blank}\downarrow\tel M
&:= \dhomj{\Fstsigma H}{\apd{(x.\fst x)}{N_1}}{\apd{(x.\fst x)}{N_2}}{\blank}
\downarrow \tel M \\
\Apd{(x.\fst x)}{(\dhomj H{N_1}{N_2}\blank)}\downarrow(\tel M,N)
&:= \dhomj{\Apd{(x.\fst x)} H}{\apd{(x.\fst x)}{N_1}}{\apd{(x.\fst x)}{N_2}}{\blank}
\downarrow (\tel M,N)
\end{aligned}\]
...
\begin{lem}
$\deqdepglob\Gamma{\Sndsigma H}{(\tel M,\apd{(x.\fst x)}N)}{\Apd{(x.\snd x)}H}{(\tel M,N)}$ for any
$\ofdepglob\Gamma NH{\tel M}$.
\end{lem}
\begin{proof}
There are two cases. If $H=\teltm xA.(\sigmacl xAB)$,
\[\begin{aligned}
\Sndsigma{\teltm xA.(\sigmacl xAB)}\downarrow(\tel M,\apd{(x.\fst x)}N)
&:= \teltm xA,\tptm xA.B\downarrow(\tel M,\fst N) \\
\Apd{(x.\snd x)}{(\teltm xA.(\sigmacl xAB))}\downarrow(\tel M,N)
...
\[\begin{aligned}
\Sndsigma{\dhomj H{N_1}{N_2}\blank}\downarrow\tel M
&:= \dhomj{\Sndsigma H}{\apd{(x.\snd x)}{N_1}}{\apd{(x.\snd x)}{N_2}}{\blank}
\downarrow\tel M \\
\Apd{(x.\snd x)}{(\dhomj H{N_1}{N_2}\blank)}\downarrow(\tel M,N)
&:= \dhomj{\Apd{(x.\snd x)} H}{\apd{(x.\snd x)}{N_1}}{\apd{(x.\snd x)}{N_2}}{\blank}
\downarrow(\tel M,N)
\end{aligned}\]
By the inductive hypothesis these are the same.
\end{proof}
